...
export const TextareaWrapper = styled(TextAreaWrapper)(() => {
  return css`
    height: fit-content;
    position: relative;
  `;
...
export const TextArea = styled.textarea(({ sizeKey }: { sizeKey: SizeKey }) => {
  return css`
    width: 100%;
    outline: none;
    resize: none;
...
export const Loading = styled(TextAreaWrapper)(({
  sizeKey,
}: {
  sizeKey: SizeKey;
}) => {
